...
function Settings:setup_about()
  ---@type widget.label
  local title = Label(self.about, "ArcCode")
  title.font = "big_font"
  ---@type widget.label
  local version = Label(self.about, "LiteXL version " .. VERSION)
  ---@type widget.label
  local description = Label(
    self.about,
    "A lightweight text editor liteXL for Windows Experience."
  )
  ...
  function self.about:update_positions()
    local centerx = self:get_width() / 2
    local centery = self:get_height() / 2

    title:set_label("ArcCode")
    title:set_position(
      centerx - (title:get_width() / 2),
      centery - (title:get_width() / 1.5)
    )

    version:set_position(
      centerx - (version:get_width() / 2),
      title:get_bottom() + (style.padding.y / 2)
    )

    description:set_position(
      centerx - (description:get_width() / 2),
      version:get_bottom() + (style.padding.y / 2)
    )

    button:set_position(
      centerx - (button:get_width() / 2),
